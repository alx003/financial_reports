...
def _build_columns(periods: list[PeriodKey]) -> list[tuple[str, PeriodKey | None, bool]]:
    ...
    Periods whose fiscal_period is a synthetic, unresolved YTD marker (e.g.
    "Q3_YTD" -- a cumulative figure that couldn't be converted to a
    standalone quarter, see period_extraction.derive_missing_quarters) are
    deliberately excluded from this grid: mixing a 9-month cumulative
    figure into a row of single-quarter columns would misrepresent it as
    one more quarter rather than a different kind of period. The data
    itself is not lost -- it's still in all_series and the CSV export --
    just not placed in this specific quarter-by-quarter table."""
    ...
    for fy in sorted(by_fy):
        quarters = sorted([p for p in by_fy[fy]
                            if p.fiscal_period in ("Q1", "Q2", "Q3", "Q4")],
...
def _collect_quarterly_periods(periods: list[PeriodKey], count: int) -> list[PeriodKey]:
    ...
    totals, no unresolved YTD markers), oldest-to-newest, for the
    trend-chart window."""
