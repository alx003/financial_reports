...
INCOME_STATEMENT: tuple[LineItem, ...] = (
    ...
    # NOTE: many filers (FedEx included) report a single combined "Interest,
    # net" line rather than separate interest expense/income lines.
    # InterestIncomeExpenseNet is checked FIRST for both rows below so a
    # combined figure is captured; if a filer instead reports them
    # separately, the more specific tags take priority. Putting the same
    # net tag in both rows means whichever one is filed, the model shows
    # the net interest figure under "Interest Expense" -- check the actual
    # filing if you need the gross expense/income split.
...
CASH_FLOW: tuple[LineItem, ...] = (
    ...
    LineItem("Capital Expenditures", ("PaymentsToAcquirePropertyPlantAndEquipment",
    ...
    LineItem("Repayments of Debt", ("RepaymentsOfLongTermDebt",
                                    "RepaymentsOfLongTermDebtAndCapitalSecurities"), "cash_flow"),
