...
## Cash flow statement: quarterly columns may be blank by design

Some filers -- FedEx among them, confirmed against its real FY2026 Q3
10-Q -- only report cash flow statement figures on a **year-to-date**
basis (e.g. "Nine Months Ended February 28, 2026"), never as a standalone
single quarter. Unlike the income statement (where a standalone quarter can
almost always be derived: `Q3 = 9-month YTD - 6-month YTD`), a cash flow
statement with *no* prior-quarter YTD facts at all has nothing to subtract
from -- there's no way to honestly produce a standalone-quarter figure.

When this happens, `period_extraction.py` does **not** discard the data or
fabricate a quarterly split. It keeps the cumulative figure under its own
distinct period type (e.g. `Q3_YTD 2026`) so it's still available in
`dataset/*.csv` for year-to-date analysis. It's deliberately **excluded**
from the quarter-by-quarter grid in `outputs/*.xlsx` (Income
Statement/Balance Sheet/Cash Flow Statement tables and the Dashboard trend
charts), since placing a 9-month cumulative number in a single-quarter
column would misrepresent it. This means cash flow statement rows in the
Excel quarterly grid can be legitimately, permanently blank for filers with
this reporting pattern -- check `dataset/*.csv` for the YTD figures instead.
